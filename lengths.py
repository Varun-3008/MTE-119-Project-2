    ...
    for q1 in range(0,180):
        ...
        for q2 in range(a,b):
            ...
            for pos in positions:
                ...
                if np.sqrt((x - pos['x'])**2 + (y - pos['y'])**2) < 0.001: #threshold of 0.001
...
if __name__ == "__main__":
    ...
    with conc.ThreadPoolExecutor() as executor:
        futures = []
        for l1 in np.linspace(0.005, 5, num=10):#reduce num for less brute force at first
            for l2 in np.linspace(0.005, 5, num=10):
                for l3 in np.linspace(0.005, 0.84852813742, num=10): #0.0848... is max length of l3
